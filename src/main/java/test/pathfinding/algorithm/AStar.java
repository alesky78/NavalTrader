		...
		while (!finish) {
			...
			for (Cell cell : adjacentNodes) {
				if(!cell.isOpen()) {
					...
					cell.setgCosts(actualCell.getgCosts()+1); 	// set g costs of this node (costs from start to this node)
					cell.setOpen(true);							//set node open		
					open.add(cell);								// add node to openList
				} else { // node is in openList
					if (cell.getgCosts() > (cell.getgCosts()+1)) { 		// costs from current node are cheaper than previous costs
						cell.setPrevious(actualCell);					// set current node as previous for this node
						cell.setgCosts(actualCell.getgCosts()+1); 		// set g costs of this node (costs from start to this node)
					}
				}
